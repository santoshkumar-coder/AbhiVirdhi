...
const Wheeler_Information: React.FC<WheelerInformationProps> = ({ data }) => {
  ...
  const { id, serviceInformation } = useParams<{
    id: string;
    ...
    <div className="font-titillium">
      <div className="flex flex-col justify-center items-center py-10 px-5">
        ...
        <div className="md:w-[70%] p-5 md:flex border-2 border-gray-300 my-10 bg-white rounded-lg gap-5">
          ...
          <div>
            <img
              className="w-[50rem] h-[15rem] object-cover"
              src={data[0]?.image}
              alt={data[0]?.name}
            />
          </div>
          ...
          <div>
            ...
            <span className="flex items-center gap-3 bg-blue-200 w-fit py-1 px-2 rounded-lg">
              <IoBag /> {data[0]?.weight} - {data[0]?.max_weight} Kgs
            </span>

            {/* <h1 className="text-xl my-2">
              Starting from{" "}
              <span className="font-semibold text-2xl">
                ₹{data[0]?.starting_price}
              </span>
            </h1> */}
            ...
            <h1
              className="mt-5 underline text-blue-700 hover:text-blue-500 cursor-pointer select-none font-bold text-xl w-fit"
