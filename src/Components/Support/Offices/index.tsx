  ...
  return (
    <div className="bg-black font-titillium text-white flex flex-col items-center justify-center py-10">
      ...
      <div className="flex flex-col items-center justify-center my-20">
        <h1 className="text-3xl font-bold text-white/90 uppercase">
          Head Office
        </h1>
        <p className="w-10 h-1 mt-1 bg-white/90 rounded-full" />
      </div>
      <div className="md:w-[40%]">
        <div className="md:w-[60%] md:px-0 px-10">
          <h1 className="text-2xl font-semibold">
            {data && data[0]?.headoffice_state_name}
          </h1>

          <p className="my-5 text-justify">
            {data && data[0]?.headoffice_address}
          </p>

          <a
            href={`https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(
              data?.[0]?.headoffice_address || "",
            )}`}
            target="_blank"
      ...
      <div className="md:w-[60%] flex justify-between flex-wrap gap-16 md:px-0 px-10">
        {data &&
          data[0].addresses?.map((item, index) => {
            return (
