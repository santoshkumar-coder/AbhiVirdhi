...
const GetEstmate: React.FC<EstimateProps> = ({ setEstimates }) => {
  ...
  return (
    ...
    >
      <div
        className={`relative md:w-1/4 md:h-screen h-1/3 bg-gray-200 text-gray-800 transform transition-all duration-700 ${
          show
      ...
      >
        <div
          className="absolute md:hidden right-3 top-2 text-3xl bg-black text-white rounded-full flex justify-center items-center px-2"
          onClick={(e) => handleEstimateClose(e)}
        >
          <p className="text-3xl -mt-1" onClick={(e) => handleEstimateClose(e)}>
            x
          </p>
        </div>
        ...
        <div className="md:py-20 py-10 md:px-10 px-5">
          {!selectedVeical && (
            <h1 className="text-4xl font-bold ">Get an Estimate</h1>
          )}
          ...
          {selectedVeical === "Three Wheeler" && (
            <h1 className="text-4xl font-bold ">Three Wheeler</h1>
          )}
          {selectedVeical === "Large Trucks" && (
            <h1 className="text-4xl font-bold ">Large Trucks</h1>
          )}
      ...
      <div
        className={`md:w-2/5 md:h-screen h-2/3 bg-white transition-all duration-700 transform ${
          show
            ? "md:translate-x-0 translate-y-0 opacity-100"
            : "md:translate-x-full translate-y-full opacity-0"
        }`}
        ...
        <div className="md:py-20 py-5 px-3 ">
          {!selectedVeical ? (
            <div>
              {data?.map((item, index) => (
                <div
                  className="h-20 rounded-xl mt-5 flex border-2 border-gray-400 hover:border-blue-700 text-gray-800 hover:text-blue-500 cursor-pointer select-none px-2 items-center justify-between"
                  onClick={() => {
              ...
              {(!serviceInformation || !serviceId) && (
                <div className="px-5 select-none">
                  <div className="flex justify-between items-center border-2 border-gray-200 py-2 px-5 rounded-lg">
                    ...
                    <p
                      className="font-semibold text-blue-500 text-sm cursor-pointer hover:text-blue-700"
                      onClick={() => {
                        setSelectedVeical("");
                        setVicalDescription("");
                      }}
